...
function searchList2 (list, query) {
    function searchCallback2 (object) {
        return (
            object.name.toLowerCase().includes(query.toLowerCase()) || //Search if the object name (transferred to lower case) contains the query, OR
            object.description.toLowerCase().includes(query.toLowerCase()) || //Same as above but in description
            object.tags.find((tag) => tag.toLowerCase().includes(query.toLowerCase())) //Same as above but using an array, so we use the find. (I think)
        );
    }
    const filtered = list.filter(searchCallback2);

    const sorted = filtered.sort((a, b) => a.distance > b.distance); //sort the accumulated list by distance, larger distance first
    return sorted;
}
